...
import { loadUserProgress, saveUserProgress, buildProgressPayload } from './services/progressService';
...
import { VALID_VIEWS, pathFromView } from './lib/viewRouting';
...
const App = () => {
  ...
  // Store state
  const {
    view, setView, currentLessonIndex, xp, setXp, activeAtmosphere, musicConfig,
    streak, completedLessons, hydrateProgress, checkAndUpdateStreak, resetSessionState
  } = useAppStore();
  
  const { setIsPlaying, triggerSupernova } = useImmersionStore();
  ...
  const [isHydratingProgress, setIsHydratingProgress] = React.useState(true);

  const authUser = identity.user;
  const authLoading = identity.isLoading;

  // Streak & Auth Side Effects
  useEffect(() => {
    if (identity.isAuthenticated) checkAndUpdateStreak();
  }, [identity.isAuthenticated, checkAndUpdateStreak]);
  ...
  useEffect(() => {
    ...
    loadUserProgress(authUser.id).then(data => {
      if (data) hydrateProgress({
        ...data,
        view: typeof data.view === 'string' && VALID_VIEWS.has(data.view) ? data.view : undefined,
      });
      setIsHydratingProgress(false);
    });
  }, [authUser?.id, hydrateProgress]);

  useEffect(() => {
    if (!authUser?.id || isHydratingProgress) return;
    const timeout = setTimeout(() => {
      saveUserProgress(authUser.id, buildProgressPayload({
        xp, completedModules: completedLessons, view, musicConfig, activeAtmosphere
      }));
    }, 900);
    return () => clearTimeout(timeout);
  }, [authUser?.id, xp, completedLessons, view, musicConfig, activeAtmosphere]);
  ...
  const { focusedPlanet, setFocusedPlanet } = useImmersionStore();
  ...
  return (
    <div className="flex h-screen bg-void text-ghost font-sora overflow-hidden relative"
      ...
      <Universe3D
        ...
        onPlanetClick={(p) => { setFocusedPlanet(p); setXp(x => x + 1); }}
      ...
      {view !== 'onboarding' && (
        <Sidebar
          ...
          lessons={lessons} view={view} setView={setView} handleLogout={identity.signOut}
          {...chat} handleReset={handleReset}
        />
      )}

      <main className={`flex-1 overflow-y-auto relative z-10 pointer-events-none ${view === 'onboarding' ? 'flex items-center justify-center' : 'pb-32 px-6 lg:px-12 pt-12'}`}>
